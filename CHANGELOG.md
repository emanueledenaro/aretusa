...
- Foundations: Badge with soft and outline variants, an icon and a status dot; Card with explicit anatomy (`CardHeader` with an action slot, `CardTitle`, `CardDescription`, `CardContent`, `CardFooter` with alignment); Button Group with optional shared borders and vertical orientation (`label` is now optional); Kbd with `keys`, spoken glyph names and `kbdKeyName`; Separator with orientation, decorative mode, label and spacing, drawn as a background so theme border resets cannot alter it; Aspect Ratio with `fit` and a fallback ratio for invalid values; Direction now propagates to overlays rendered in a portal and exposes `useDirection`.
- Foundations: Skeleton with shapes, lines and `SkeletonGroup` (text shape now carries 0.25em vertical margins); Spinner with sizes and an own SVG that stays static under reduced motion; Progress as a `progressbar` with indeterminate state, `max`, `description`, `formatValue`, `labelHidden` and `tone` (`value` is now optional, styles ship in `progress.css`); Empty with variants, heading level and compact mode (place buttons in `action`, not `children`); Item with leading, meta, links with an extended activation area, selected and disabled states and stacked actions in narrow containers; Alert with a warning tone, icons, an action, dismissal and a `live` setting; Typography with `variant`, `as` up to h6, `muted` and fluid display sizes.
