...
Two defects found in the browser during integration and fixed before the commit: a long label kept `shrink-0`, so the labelled rule widened the page to 400 px inside a 240 px parent (now `min-w-0` on the row and the label, with a regression test); and the demo placed the vertical rule inside a `p`, which React rejects as a nested div.
