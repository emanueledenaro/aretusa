...
Bring Badge to the Aretusa design and code quality contract, from rendered behavior through documented, installable source. Existing code is a starting point, not evidence of completion.

## Component-specific scenarios

- [ ] Tune compact typography, inset spacing and icon/text alignment.
- [ ] Verify semantic state labels and light/dark contrast without color-only meaning.
- [ ] Test long status labels, wrapping and placement beside controls.
