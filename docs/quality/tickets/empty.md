...
Bring Empty to the Aretusa design and code quality contract, from rendered behavior through documented, installable source. Existing code is a starting point, not evidence of completion.

## Component-specific scenarios

- [ ] Make absence understandable with a useful next step and measured visual hierarchy.
- [ ] Distinguish no data, no search results, permission limits and error recovery.
- [ ] Test long explanations and primary actions on mobile.
