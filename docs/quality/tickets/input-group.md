...
Bring Input Group to the Aretusa design and code quality contract, from rendered behavior through documented, installable source. Existing code is a starting point, not evidence of completion.

## Component-specific scenarios

- [ ] Align prefix, suffix, icon and action content without shrinking the input excessively.
- [ ] Preserve input labeling and ensure addon actions do not submit accidentally.
- [ ] Test narrow containers, long prefixes, error focus and touch controls.
...
Prerequisite items: input. Native blocking links will be added after the full catalog is registered.

## Evidence

Status remains implemented until behavior, visual and consumer gates have evidence. Use docs/quality/evidence-template.md.

Aretusa-Item: input-group
