...
Bring Spinner to the Aretusa design and code quality contract, from rendered behavior through documented, installable source. Existing code is a starting point, not evidence of completion.

## Component-specific scenarios

- [ ] Balance stroke, size and baseline with nearby text and buttons.
- [ ] Expose a concise loading status without repeated announcements.
- [ ] Provide equivalent static feedback with reduced motion.
