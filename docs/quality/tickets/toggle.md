...
Bring Toggle to the Aretusa design and code quality contract, from rendered behavior through documented, installable source. Existing code is a starting point, not evidence of completion.

## Component-specific scenarios

- [ ] Give pressed and unpressed states equal clarity without relying on color alone.
- [ ] Verify aria-pressed, keyboard activation and disabled behavior.
- [ ] Test icon-only labels and touch sizing in toolbar compositions.
