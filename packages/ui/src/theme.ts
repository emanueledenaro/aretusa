...
export type ThemeConfig = {
  style: (typeof styleNames)[number];
...
export function validateTheme(input: unknown): ThemeConfig {
  ...
  const v = { ...defaultTheme, ...input } as ThemeConfig;
  if (
    !styleNames.includes(v.style) ||
  ...
  return Object.fromEntries(
    Object.keys(defaultTheme).map((k) => [k, v[k as keyof ThemeConfig]]),
  ) as ThemeConfig;
}
export function themeTokens(
  ...
  const base = bases[c.base],
    paper = dark ? "#191b18" : base[0],
    line = dark ? "#454b40" : base[3];
  const accents = {
  ...
  const s = styles[c.style],
    card = c.style === "riva" ? paper : dark ? "#222520" : base[1];
  return {
    "--color-paper": paper,
    "--color-card": card,
...
export function themeCSS(config: ThemeConfig) {
  return (
    ":root {\n" +
