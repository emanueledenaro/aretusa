...
for(const item of catalog){const code=usageCode(item.id,item.exportName,item.module).replace('./components/aretusa/','../packages/ui/src/');await writeFile('.usage-check/'+item.id+'.tsx',code);}
