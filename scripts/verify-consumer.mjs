...
run(process.execPath, [cli, "add", "formisch", "--cwd", consumer]);
...
const required = new Set([
  ...
  "react-dom",
  ...registry.items
    .filter((item) => ["dialog", "field", "shimmer", "scroll-fade", "react-hook-form", "tanstack-form", "formisch"].includes(item.name))
...
await writeFile(
  ...
  "import React from 'react';import {Shimmer} from './components/aretusa/shimmer';import {ScrollFade,useScrollFade} from './components/aretusa/scroll-fade';import {useForm} from 'react-hook-form';import {HookFormField} from './components/aretusa/react-hook-form';import {useForm as useTanStackForm} from '@tanstack/react-form';import {TanStackFormField,focusFirstInvalidField} from './components/aretusa/tanstack-form';import {Form as FormischForm,useForm as useFormischForm} from '@formisch/react';import * as v from 'valibot';import {FormischField} from './components/aretusa/formisch';import {createRoot} from 'react-dom/client';import {Button} from './components/aretusa/button';import {Modal} from './components/aretusa/overlays';import {Input,Field} from './components/aretusa/forms';import './components/aretusa/styles.css';function Profile(){const form=useForm<{name:string}>({defaultValues:{name:''}});return <form noValidate onSubmit={form.handleSubmit(()=>{})}><HookFormField control={form.control} name='name' label='Name' rules={{required:'Enter your name.'}}>{({field,controlProps})=><Input {...field} {...controlProps}/>}</HookFormField></form>;}function Visit(){const form=useTanStackForm({defaultValues:{name:''},onSubmitInvalid:({formApi})=>{setTimeout(()=>focusFirstInvalidField(formApi),0);}});return <form noValidate onSubmit={(e)=>{e.preventDefault();void form.handleSubmit();}}><form.Field name='name' validators={{onChange:({value})=>value?undefined:'Enter your name.'}}>{(field)=><TanStackFormField field={field} label='Name'>{({controlProps})=><Input {...controlProps} name={field.name} value={field.state.value} onChange={(e)=>field.handleChange(e.target.value)} onBlur={field.handleBlur}/>}</TanStackFormField>}</form.Field></form>;}function Booking(){const form=useFormischForm({schema:v.object({name:v.pipe(v.string(),v.nonEmpty('Enter your name.'))}),initialInput:{name:''}});return <FormischForm of={form} onSubmit={()=>{}}><FormischField of={form} path={['name']} label='Name'>{({field,controlProps})=><Input {...field.props} {...controlProps} value={field.input??''}/>}</FormischField></FormischForm>;}function Activity(){const {ref,edges}=useScrollFade({axis:'both'});return <div className='relative'><div ref={ref} role='region' aria-label='Activity' tabIndex={0} className='h-20 overflow-auto'><p>Installed scroll fade</p></div><ScrollFade edges={edges} depth={24}/></div>;}createRoot(document.getElementById('root')!).render(<Modal trigger={<Button>Open</Button>} title='Consumer' description='Installed source'><Shimmer>Loading preview</Shimmer><Activity/><Profile/><Visit/><Booking/><Field label='Name'><Input/></Field></Modal>);",
);
